...
impl Hsla {
    ...
    pub fn from_hsva(hsva: Hsva) -> Self {
        ...
        Self {
            ...
            saturation: if lightness == 0.0 || lightness == 1.0 {
                ...
                (hsva.value() - lightness) / lightness.min(1.0 - lightness)
            },
            lightness,
            alpha: hsva.alpha(),
        }
    ...
    pub fn from_rgba(rgba: sRgba<f32>) -> Self {
        rgba.to_hwba().to_hsva().to_hsla()
    }

    pub fn to_hsva(&self) -> Hsva {
        let value =
            self.lightness() + self.saturation() * self.lightness().min(1.0 - self.lightness());
        Hsva::new(
            self.hue(),
            if value == 0.0 {
                0.0
            } else {
                2.0 * (1.0 - self.lightness() / value)
            },
            value,
            self.alpha(),
        )
