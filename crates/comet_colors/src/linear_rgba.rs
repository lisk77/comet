...
impl LinearRgba {
    ...
    pub fn from_rgba(srgba: sRgba<f32>) -> Self {
        Self {
            red: if srgba.red() <= 0.04045 {
                srgba.red() / 12.92
            } else {
                ((srgba.red() + 0.055) / 1.055).powf(2.4)
            },
            green: if srgba.green() <= 0.04045 {
                srgba.green() / 12.92
            } else {
                ((srgba.green() + 0.055) / 1.055).powf(2.4)
            },
            blue: if srgba.blue() <= 0.04045 {
                srgba.blue() / 12.92
            } else {
                ((srgba.blue() + 0.055) / 1.055).powf(2.4)
            },
            alpha: srgba.alpha(),
        }
    }
    ...
    pub fn to_rgba(&self) -> sRgba<f32> {
        sRgba::<f32>::new(
            if self.red <= 0.0031308 {
                ...
                1.055 * self.red.powf(1.0 / 2.4) - 0.055
            },
            if self.green <= 0.0031308 {
                self.green * 12.92
            } else {
                1.055 * self.green.powf(1.0 / 2.4) - 0.055
            },
            if self.blue <= 0.0031308 {
                self.blue * 12.92
            } else {
                1.055 * self.blue.powf(1.0 / 2.4) - 0.055
            },
            self.alpha,
        )
    }
