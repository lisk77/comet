...
impl sRgba<f32> {
    ...
    pub fn from_linear(linear: LinearRgba) -> Self {
        Self {
            red: if linear.red() <= 0.0031308 {
                12.92 * linear.red()
            } else {
                1.055 * linear.red().powf(1.0 / 2.4) - 0.055
            },
            green: if linear.green() <= 0.0031308 {
                12.92 * linear.green()
            } else {
                1.055 * linear.green().powf(1.0 / 2.4) - 0.055
            },
            blue: if linear.blue() <= 0.0031308 {
                12.92 * linear.blue()
            } else {
                1.055 * linear.blue().powf(1.0 / 2.4) - 0.055
            },
            alpha: linear.alpha(),
        }
    }
    ...
    pub fn to_linear(&self) -> LinearRgba {
        LinearRgba::new(
            if self.red() <= 0.04045 {
                ...
                ((self.red() + 0.055) / 1.055).powf(2.4)
            },
            if self.green() <= 0.04045 {
                self.green() / 12.92
            } else {
                ((self.green() + 0.055) / 1.055).powf(2.4)
            },
            if self.blue() <= 0.04045 {
                self.blue() / 12.92
            } else {
                ((self.blue() + 0.055) / 1.055).powf(2.4)
            },
            self.alpha(),
        )
    }
