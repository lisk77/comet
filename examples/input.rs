...
fn update(app: &mut App, _dt: f32) {
    ...
    for button in [Button::Left, Button::Right, Button::Middle] {
        if app.mouse_pressed(button) {
            info!("Mouse pressed:  {:?}", button);
        }
        if app.mouse_released(button) {
    ...
    if app.cursor_exited() {
        info!("Cursor exited window");
    }
    ...
    for id in app.connected_gamepads() {
        for button in [
            ...
            if app.gamepad_button_pressed(id, button) {
                info!("Gamepad {:?} pressed:  {:?}", id, button);
            }
            if app.gamepad_button_released(id, button) {
                info!("Gamepad {:?} released: {:?}", id, button);
            }
        }
